...
export function CalorieTracker() {
  ...
  const handleFitnessSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      // If no Groq API key is configured, fall back to a simple local plan
      if (!process.env.NEXT_PUBLIC_GROQ_API_KEY) {
        const weight = parseFloat(fitnessFormData.weight || '70');
        const height = parseFloat(fitnessFormData.height || '170');
        const age = parseFloat(fitnessFormData.age || '25');

        // Basic Mifflin-St Jeor estimate
        const bmr =
          (fitnessFormData.gender === 'female'
            ? 10 * weight + 6.25 * height - 5 * age - 161
            : 10 * weight + 6.25 * height - 5 * age + 5) || 2000;

        const activityFactors: Record<FitnessFormData['activityLevel'], number> = {
          sedentary: 1.2,
          light: 1.375,
          moderate: 1.55,
          very: 1.725,
        ...
        const goalAdjustments: Record<FitnessFormData['goal'], number> = {
          'weight-loss': -300,
          'muscle-gain': 300,
          maintenance: 0,
          endurance: 100,
        ...
        const maintenance = bmr * (activityFactors[fitnessFormData.activityLevel] || 1.3);
        const dailyCalories = Math.round(maintenance + (goalAdjustments[fitnessFormData.goal] || 0));

        const macroSplit: MacroSplit =
          fitnessFormData.goal === 'muscle-gain'
            ? { protein: 35, carbs: 40, fats: 25 }
            : fitnessFormData.goal === 'weight-loss'
              ? { protein: 35, carbs: 35, fats: 30 }
              : { protein: 30, carbs: 45, fats: 25 };

        const localPlan: FitnessPlan = {
          dailyCalories: `${dailyCalories}`,
          macroSplit,
          workoutPlan:
            '<h4>Weekly Workout Schedule</h4><ul><li>3–4x/week full‑body strength training</li><li>2–3x/week light cardio (20–30min)</li></ul>',
          mealPlan:
            '<h4>Sample Day</h4><ul><li>Breakfast: Oats + eggs</li><li>Lunch: Rice, dal, vegetables</li><li>Dinner: Lean protein + salad</li></ul>',
          tips: [
            'Sleep 7–8 hours per night.',
            'Aim for at least 7000–8000 steps daily.',
            'Keep hydration high (2–3L water).',
          ],
        };

        setFitnessPlan(localPlan);
        return;
      }
...
Goal: ${fitnessFormData.goal}`,
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      // If no Groq API key is configured, return a simple local estimate instead of calling external APIs
      if (!process.env.NEXT_PUBLIC_GROQ_API_KEY) {
        const fallbackNutrition: NutritionData = {
          ...
          fat: '10',
          analysis:
            '<h4>Nutritional Analysis (Estimate)</h4><p>This is an approximate analysis generated locally because no AI API key is configured. Treat values as rough guidance only.</p>',
        };

        setNutritionData(fallbackNutrition);

        const foodData = {
          food: 'Scanned food (approximate)',

...
Focus on authentic ${cuisine} recipes only. DO NOT include code blocks, markdown formatting, or any explanatory text - JUST the JSON object exactly as specified.`;
    ...
    // If Gemini integration is disabled (or rate-limited / causing issues), we can
    // skip the external API entirely and just serve high-quality mock recipes.
    // Toggle with ENABLE_GEMINI_RECIPES=true in .env.local if you want to use the real API.
    const enableGemini = process.env.ENABLE_GEMINI_RECIPES === 'true';

    // If Gemini is disabled, immediately return mock data customized to the request.
    if (!enableGemini) {
    ...
    try {
      ...
      // Use the v1beta endpoint and a current recommended model
      ...
      if (!geminiResponse.ok) {
        ...
        // Log once, then fall back to mock data without throwing
        if (status === 400) {
          ...
          console.error('Gemini API error: Model not found or not enabled for this key.', errorText);
        } else if (status === 429) {
        ...
        // Let outer catch block handle and return mock data
